...
class BBPO_Purchase_Orders_Files {
	 ...
	 * @return void
	 */
	public function send_success_signal( $data = array() ) {
	 ...
	 * @return void
	 */
	public function send_error_signal( $data = array() ) {
	...
	public function validate_file_path( $file_path ) {
		...
		if ( ! empty( $file_path ) ) {
			$upload_dir = wp_upload_dir();
			if ( strpos( $file_path, $upload_dir['basedir'] . '/wc-purchase-orders' ) === 0 ) {
				$valid = true;
			}
		}
		return $valid;
	 ...
	 * @return mixed
	 */
	public function check_filetypes( $data, $file, $filename, $mimes, $real_mime ) {
